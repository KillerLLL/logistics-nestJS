 ...
 * 收货地址（卸货地）实体。
 *
 * 字段结构与发货地址（Address / gy_company_load_addr）完全一致，
 * 仅表名不同：gy_company_unload_addr。
 * 前端通过 /logistics-boot/company/gyCompanyUnLoadAddr/ 访问。
 */
@Entity('gy_company_unload_addr')
@Index('idx_company_id', ['companyId'])
export class UnloadAddress {
  ...
  @Column({ type: 'varchar', length: 100, nullable: true, comment: '卸货地名称' })
  name: string;
